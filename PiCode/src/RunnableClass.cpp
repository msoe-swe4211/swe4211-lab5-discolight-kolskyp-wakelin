 ...
 */
RunnableClass::RunnableClass(std::string threadName,  int priority) {
...
void RunnableClass::invokeRunMethod() {
	...
	if (priority > 0) {
		if (priority > sched_get_priority_max(RT_SCHEDULER_TO_USE)) {
			p.__sched_priority = sched_get_priority_max(RT_SCHEDULER_TO_USE);
		} else {
			p.__sched_priority = priority;
		}

		if (sched_setscheduler(0, RT_SCHEDULER_TO_USE, &p) != 0) {
...
// TODO
void RunnableClass::stopChildRunnables() {
	/**
	 * This default implementation does nothing.
	 */
}
